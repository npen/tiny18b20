...
override CFLAGS        = -g -I./avr-onewire -std=gnu99 -DF_CPU=$(HZ) -Wall $(OPTIMIZE) -mmcu=$(MCU_TARGET) $(DEFS)
override LDFLAGS       = -Wl,-Map,$(PRG).map -L./avr-onewire

OBJCOPY        = avr-objcopy
OBJDUMP        = avr-objdump

all: libonewire $(PRG).elf lst text #eeprom
